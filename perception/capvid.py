...
CAPTURE_TIME:int = 10*CAMERA_SECOND
VID_FNAME:str = f'cam{CAMERA_IDX}cap.avi'

capture = syscamera.config.setupCamera(CAMERA_IDX)
width:int = int(capture.get(3))
height:int = int(capture.get(4))
size = (width, height)
writer = cv.VideoWriter(VID_FNAME, cv.VideoWriter_fourcc(*'MJPG'), 10, frameSize=size)
